...
fun GameBoard<Int?>.moveValuesInRowOrColumn(rowOrColumn: List<Cell>): Boolean {
    val updatedValues: List<Int> = rowOrColumn.map { this[it] }
        .moveAndMergeEqual { value -> value * 2 }
    rowOrColumn.forEachIndexed { index, cell ->
        this[cell] = if (index < updatedValues.size) updatedValues[index] else null
    }
    return updatedValues.isNotEmpty() && updatedValues.size < rowOrColumn.size
}
...
fun GameBoard<Int?>.moveValues(direction: Direction): Boolean {
    ...
    (1..width).forEach { index ->
        ...
        val currentMove = moveValuesInRowOrColumn(rowOrColumn)
        moved = moved || currentMove
    }
    return moved
}